...
class Downsampling_Stem(nn.Module):
    # Stem
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
...
class modified_lite_vision_transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        out_dict = {}
        out_dict["dark2"] = dark2
        out_dict["dark3"] = dark3
        out_dict["dark4"] = dark4
        out_dict["dark5"] = dark5
...
rasa_cfg = DotDict(dict(
    atrous_rates=[1, 3, 5,7],
...
@register_model
class M_LVT(modified_lite_vision_transformer):
    def __init__(self, rasa_cfg=rasa_cfg, with_cls_head=False, **kwargs):
        super().__init__(
            layers=[1, 1, 2, 2],
            patch_size=4,
